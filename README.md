...
- **Theme system with light/dark.** All colours flow from a `useColors()` hook; theme preference persists via AsyncStorage. Components consume tokens (`Greyscale[0]` = surface, `Greyscale[900]` = primary text) so they swap correctly when the theme flips. Trading-specific accents: Buy uses the brand orange (`primary[500]`) for on-brand emphasis; Sell uses `Alert.Error` shades selected per theme.
